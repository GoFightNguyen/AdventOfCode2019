...
type Puzzle01Tests () =
    ...
    [<TestMethod>]
    member this.CanParsePath () =
        let path = [|"R8";"U5";"L5";"D10"|]
        let expected = [|
    ...
    member this.CanMapWire_Path1 () =
        ...
        let pathPoints = MapWire path
        let areEqual = expected = pathPoints
        Assert.IsTrue areEqual

    [<TestMethod>]
    member this.CanMapWire_Path2 () =
    ...
    [<TestMethod>]
    member this.CanFindIntersectingPointsOfDifferentWires () =
        let path1 = [|"R8";"U5";"L5";"D3"|]
        let path2 = [|"U7";"R6";"D4";"L4"|]
        let pathPoints1 = MapWire path1
        let pathPoints2 = MapWire path2

        let expected = Set.ofList [5,6; 3,3]
        let actual = FindIntersectingPoints pathPoints1 pathPoints2

        let areEqual = expected = actual
        Assert.IsTrue areEqual

    [<TestMethod>]
    member this.CanCalculateTheManhattanDistance () =
        let points = Set.ofList [5,6; 3,3]
        let expected = Set.ofList [11; 6]
        let actual = points |> Set.map CalculateManhattanDistance
        let areEqual = expected = actual
        Assert.IsTrue(areEqual)

    // example 1
    [<TestMethod>]
    member this.CanFindTheIntersectionPointClosestToTheCentralPort () =
        let path1 = [|"R8";"U5";"L5";"D3"|]
        let path2 = [|"U7";"R6";"D4";"L4"|]
        let pathPoints1 = MapWire path1
        let pathPoints2 = MapWire path2
        ...
        let closest =
            pathPoints1
            |> FindIntersectingPoints pathPoints2
            |> FindClosest

        let expected = {X = 3; Y = 3; ManhattanDistanceFromCentralPoint = 6 }
    ...
    member this.Example2 () =
        ...
        Assert.AreEqual(159, closest.ManhattanDistanceFromCentralPoint)
    ...
    member this.Puzzle01Solution () =
        ...
        Assert.AreEqual(1519, closest.ManhattanDistanceFromCentralPoint)


[<TestClass>]
type Puzzle02Tests () =
    [<TestMethod>]
    member this.CanFindTheNumberOfStepsToAPoint () =
        let path1 = [|"R8";"U5";"L5";"D3"|]
        let pathPoints1 = MapWire path1
        ...
        Assert.AreEqual(20, stepsTo)

    [<TestMethod>]
    member this.CanFindTheNumberOfStepsToAnotherPoint () =
        let path1 = [|"R8";"U5";"L5";"D3"|]
        let pathPoints1 = MapWire path1
        ...
        Assert.AreEqual(15, stepsTo)

    // example 1
    [<TestMethod>]
    member this.CanFindTheMinimumCombinedStepsToAnIntersection () =
        let path1 = [|"R8";"U5";"L5";"D3"|]
        let path2 = [|"U7";"R6";"D4";"L4"|]
        let pathPoints1 = MapWire path1
        let pathPoints2 = MapWire path2

        let closestWithMinimalSignalDelay = FindClosestWithMinimalSignalDelay pathPoints1 pathPoints2

        let closest =
            pathPoints1
            |> FindIntersectingPoints pathPoints2
            |> closestWithMinimalSignalDelay

        Assert.AreEqual(30, closest)
